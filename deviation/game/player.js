function getPlayer(strategy) {
    ...
    player.movingAvg = function(win) {
        if (Math.abs(this.stats.totals.won - this.stats.totals.lost) > this.stats.maxDeviation) {
            this.stats.maxDeviation = Math.abs(this.stats.totals.won - this.stats.totals.lost);
        }
        this.stats.recentHistory.push(win);
        if (this.stats.recentHistory.length > 100) {
            this.stats.recentHistory.splice(0, 1);
        }
    };
